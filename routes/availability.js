...
router.get("/:staffId/:serviceId", async (req, res, next) => {
  ...
  try {
    ...
    if (staffId === ANY_STAFF_PARAMS) {
      console.log("Searching for availability for all staff members...");
      ...
      console.log(
        `Searching for availability for staff member with ID: ${staffId}`
      );
      searchRequest.query.filter.segmentFilters[0].teamMemberIdFilter = {
        any: [staffId],
      };
    ...
    console.log("Sending response with availabilities...");
    res.json({
      availabilities,
      serviceId,
      serviceVersion,
